...
router.post('/events', eventLimiter, async (req, res) => {
  try {
    ...
    if (!event || !/^[a-zA-Z0-9_.:-]{2,80}$/.test(event)) {
      return res.status(400).json({ ok: false, error: 'Invalid event name (2-80 chars, alphanumeric plus ._:-).' });
    }
...
router.get('/admin/summary', requireAdmin, async (req, res) => {
  try {
    ...
    const [events, uniqueSessions, devices, paths, topEvents, ctas] = await Promise.all([
      ...
      AnalyticsEvent.distinct('sessionId', { createdAt: { $gte: since } }).then(a => a.length),
      ...
      AnalyticsEvent.aggregate([
        { $match: { event: 'cta_click', createdAt: { $gte: since } } },
        { $group: { _id: '$meta.target', count: { $sum: 1 } } },
        { $sort: { count: -1 } },
        { $limit: 10 }
      ])
    ]);
    ...
    res.json({
      ok: true,
      data: { events, uniqueSessions, devices, paths, topEvents, ctas }
    });
  } catch (err) {
