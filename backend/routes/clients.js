...
const { signJwt, verifyJwt, requireAdmin } = require('../middleware/auth');
...
function client(req,res,next){
  const h=req.get('authorization')||'',t=h.startsWith('Bearer ')?h.slice(7):'';
  try{
    const p=verifyJwt(t,process.env.ADMIN_JWT_SECRET);
    if(p.role!=='client')throw 0;
    req.client=p;next();
  }catch{return res.status(401).json({ok:false,error:'Client session expired.'})}
}
router.post('/login', async (req,res)=>{
  try{
    const email=String(req.body?.email||'').trim().toLowerCase(),password=String(req.body?.password||'');
    const c=await Client.findOne({email});
    if(!c||!verifyClientPassword(password,c.passwordHash))return res.status(401).json({ok:false,error:'Invalid client credentials.'});
    const token=signJwt({role:'client',clientId:String(c._id),email:c.email},process.env.ADMIN_JWT_SECRET,12*60*60);
...
router.post('/admin',requireAdmin,async(req,res)=>{
  try{
    ...
    if(name.length<2||!email.includes('@')||password.length<8)return res.status(400).json({ok:false,error:'Name, valid email and password (8+ chars) are required.'});
    const data=await Client.create({name,email,phone:String(req.body?.phone||'').slice(0,30),passwordHash:hashPassword(password),projects:[]});
...
router.patch('/admin/:id',requireAdmin,async(req,res)=>{
  try{
    ...
    if(req.body?.password)update.passwordHash=hashPassword(String(req.body.password));
    const c=await Client.findByIdAndUpdate(req.params.id,update,{new:true}).select('-passwordHash').lean();
    if(!c)return res.status(404).json({ok:false,error:'Client not found.'});res.json({ok:true,data:c});
...
router.post('/admin/:id/projects',requireAdmin,async(req,res)=>{
  try{
    ...
    const c=await Client.findByIdAndUpdate(req.params.id,{$push:{projects:project}},{new:true}).select('-passwordHash').lean();
    if(!c)return res.status(404).json({ok:false,error:'Client not found.'});res.status(201).json({ok:true,data:c});
