...
(() => {
  ...
  if(modal){
    const title=document.getElementById('projectModalTitle'),text=document.getElementById('projectModalText'),eyebrow=document.getElementById('projectModalEyebrow'),tags=document.getElementById('projectModalTags'),preview=document.getElementById('modalPreview');
    const data={business:{eyebrow:'Business website',title:'Nova Business',text:'A conversion-first business website concept with premium visual direction, responsive layouts, lead capture and SEO-ready structure.',tags:['Responsive','Lead generation','SEO','Analytics']},store:{eyebrow:'E-commerce',title:'Mode Store',text:'An editorial e-commerce concept built around product discovery, clean merchandising and checkout-ready architecture.',tags:['Product catalogue','Payments','Orders','Admin']},creator:{eyebrow:'Portfolio',title:'Amara Studio',text:'A visual portfolio concept that puts work first, keeps the interface fast and turns visitors into project enquiries.',tags:['Portfolio','Motion','CMS','Contact funnel']}};
    document.querySelectorAll('[data-preview]').forEach(btn=>btn.addEventListener('click',()=>{const item=data[btn.dataset.preview];eyebrow.textContent=item.eyebrow;title.textContent=item.title;text.textContent=item.text;tags.innerHTML=item.tags.map(t=>`<span>${t}</span>`).join('');const source=btn.closest('.project-card').querySelector('.project-preview');preview.innerHTML=source.outerHTML;modal.classList.add('open');modal.setAttribute('aria-hidden','false');document.body.style.overflow='hidden'}));
    const close=()=>{modal.classList.remove('open');modal.setAttribute('aria-hidden','true');document.body.style.overflow=''};modal.querySelectorAll('[data-close-project]').forEach(el=>el.addEventListener('click',close));document.addEventListener('keydown',e=>{if(e.key==='Escape'&&modal.classList.contains('open'))close()});
  }
